...
export function DifficultPoint({ active, size = '10' }: { active?: boolean; size?: string }) {
  return (
    <span
      className={clsx('rounded-full inline-block', `h-[${size}px] w-[${size}px]`, active ? 'bg-secondary' : 'border-2')}
    />
  );
}
