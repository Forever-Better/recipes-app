...
export function UserNameForm({ className, user, ...props }: UserNameFormProps) {
  ...
  async function onSubmit(data: FormData) {
    ...
    const response = await fetch(getApiUrl.userUpdate(user.id), {
      method: 'PATCH',
      headers: {
      ...
      <Card>
        ...
        <CardContent>
          <div className='grid gap-1'>
            ...
            <Input className='w-[400px]' id='name' size={32} {...register('name')} />
            {errors?.name && <p className='px-1 text-xs text-red-600'>{errors.name.message}</p>}
          </div>
        </CardContent>
        <CardFooter>
          <button className={cn(buttonVariants(), className)} disabled={isSaving || !isValid} type='submit'>
